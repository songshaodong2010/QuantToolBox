...
class BrownianBridge:
    def __init__(self, steps=None, times=None, timeGrid=None):
        if steps is not None and times is None and timeGrid is None:
            self._size = steps
            self._t = self._size
            self._sqrtdt = self._size
            self._bridgeIndex = self._size
            self._leftIndex = self._size
            self._rightIndex = self._size
            self._leftWeight = self._size
            self._rightWeight = self._size
            self._stdDev = self._size
    ...
    def initialize(self):
        ...
        map = [False for i in range(self._size)]
        map[-1] = True
    ...
    def transform(self, begin, end, output):
        output[self._size - 1] = self._stdDev[0] * begin[0]
        for i in range(1, self._size):
            j = self._leftIndex[i]
            ...
            if j != 0:
                ...
                output[l] = self._rightWeight[i] * output[k] + self._stdDev[i] * begin[i]
